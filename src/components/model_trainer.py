...
from src.components.data_transformation import DataTransformation  # To get train/test arrays
...
class ModelTrainer:
    ...
    def initiate_model_trainer(self, train_array, test_array):
        try:
            ...
            logging.info(f"R2 score on test set: {r2_square}")
...
if __name__ == "__main__":
    try:
        ...
        # Paths to train/test CSVs
        train_csv = "artifacts/train.csv"
        test_csv = "artifacts/test.csv"

        # Generate train/test arrays using data transformation
        data_transformation = DataTransformation()
        train_array, test_array, _ = data_transformation.initiate_data_transformation(train_csv, test_csv)

        # Train model
        model_trainer = ModelTrainer()
        r2 = model_trainer.initiate_model_trainer(train_array, test_array)
        logging.info(f"Model training completed successfully. R2 score: {r2}")
